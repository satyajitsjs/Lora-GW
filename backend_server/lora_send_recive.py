            ...
            while self.running:
                try:
                    ...
                    try:
                        ...
                        current = float(split_values[0])
                        voltage = float(split_values[1])
                        rpm = float(split_values[2])
                        mpu = float(split_values[3])
                        node_id = int(split_values[4])
                        node_status = split_values[5]
                    ...
                    if message:
                            if c_r_c == False:
                                try:
                                    self.SaveNodeDb.save_nodeData(node_id,self.LoRa.snr(),self.LoRa.packetRssi(),counter,c_r_c,0.0,0.0,0.0,0.0,node_status)
                                    print("Lora Chnaging it's Frequency \n")
                                    self.LoRa.reset()
                                    time.sleep(1)
                                    if time.time()-self.first_time >= 60:
                                        time.sleep(1)
                                ...
                                try:
                                    self.SaveNodeDb.save_nodeData(node_id,self.LoRa.snr(),self.LoRa.packetRssi(),counter,c_r_c,current,voltage,rpm,mpu,node_status)
                                    self.LoRa.reset()
                                    time.sleep(1)
                                    if time.time()-self.first_time >= 60:
                                        time.sleep(1)
                        ...
                        self.SaveNodeDb.save_nodeData(node_id,self.LoRa.snr(),self.LoRa.packetRssi(),0,False,0,0,0,0,node_status)
                    ...
                    self.SaveNodeDb.save_nodeData(0,self.LoRa.snr(),self.LoRa.packetRssi(),0,False,0.0,0.0,0.0,0.0,False)
                    print(f"Error:{str(e)}")
                    if time.time()-self.first_time >= 60:
                        time.sleep(1)
                        self.LoRa.reset()
