...
def list_all_wifi_networks(request):
    try:
        rescan_result = subprocess.run(['sudo', 'nmcli', 'device', 'wifi', 'rescan'], stdout=PIPE, stderr=PIPE, text=True)
        if rescan_result.returncode != 0:
            return Response({"error": rescan_result.stderr}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        list_result = subprocess.run(['sudo', 'nmcli', 'device', 'wifi', 'list'], stdout=PIPE, stderr=PIPE, text=True)
...
def connect_to_wifi(request):
    if request.method == 'POST':
        ...
        password = request.data.get('password')
        try:
            subprocess.check_call(['sudo','nmcli', 'device', 'wifi', 'connect', ssid, 'password', password])
