...
class GatewayModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        existing_instances_count = GatewayModel.objects.count()
...
class MqttModel(models.Model):
    ...
    def save(self ,*args ,**kwargs):
        exicting_instace = MqttModel.objects.first()
...
class NodeDataModel(models.Model):
    ...
    @property
    def last_5_minutes_data_count(self):
        five_minutes_ago = timezone.now() - timezone.timedelta(minutes=5)
        return NodeDataModel.objects.filter(NodeId=self.NodeId, time__gte=five_minutes_ago).count()
...
def update_node_model_status(sender, instance, **kwargs):
    mode = redisLora.get("mode")
    if mode != "Sleep":
        # !Update node_status based on data count in the last 5 minutes
        if instance.last_5_minutes_data_count > 0:
            instance.NodeId.status = True
        else:
            instance.NodeId.status = False

        instance.NodeId.save()
